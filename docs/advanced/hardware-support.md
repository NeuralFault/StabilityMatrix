...
- Torch is installed from AMD's official repository (`stable.repo.amd.com/rocm/whl-next`) as device-specific wheels (`torch[device-gfxNNNN]`). Vega parts (`gfx900` / `gfx906`) pull from the nightly (`nightly.repo.amd.com/rocm/whl-next`) instead, since these architecture builds currently are only available there instead of the stable production distribution stream.
...
- **Packages:** ComfyUI, Stable Diffusion WebUI Reforge, InvokeAI, SwarmUI, OneTrainer, and Wan2GP are supported by this install path.
